...
- [第 2 部分 静态链接](#第-2-部分-静态链接)
  ...
  - [第 3 章 目标文件里有什么](#第-3-章-目标文件里有什么)
    ...
    - [3.5 链接的接口——符号](#35-链接的接口符号)
    - [4.6 调试信息](#46-调试信息)
  - [第 4 章 静态链接](#第-4-章-静态链接)
    - [4.1 空间与地址分配](#41-空间与地址分配)
    - [4.2 符号解析与重定位](#42-符号解析与重定位)
    - [4.3 COMMON 块](#43-common-块)
    - [4.4 C++ 相关问题](#44-c-相关问题)
    - [4.5 静态库链接](#45-静态库链接)
    - [4.6 链接过程控制](#46-链接过程控制)
    - [4.7 BFD 库](#47-bfd-库)
  - [第 5 章 Windows PE/COFF](#第-5-章-windows-pecoff)
    - [5.1 Windows 的二进制文件格式 PE/COFF](#51-windows-的二进制文件格式-pecoff)
    - [5.2 PE 的前身——COFF](#52-pe-的前身coff)
    - [5.6 Windows 下的 ELF——PE](#56-windows-下的-elfpe)
...
- [第 4 部分 库与运行库](#第-4-部分-库与运行库)
  ...
  - [第 11 章 运行库](#第-11-章-运行库)
    - [11.1 入口函数和程序初始化](#111-入口函数和程序初始化)
    - [11.2 C/C++ 运行库](#112-cc-运行库)
    - [11.3 运行库与多线程](#113-运行库与多线程)
    - [11.4 全局构造与析构](#114-全局构造与析构)
    - [11.5 fread 实现](#115-fread-实现)
  - [第 12 章 系统调用与 API](#第-12-章-系统调用与-api)
    - [12.2 系统调用原理](#122-系统调用原理)
...
ELF 可执行文件中有一个专门的数据结构叫做程序头表（Program Header Table），用来保存“Segment”的信息。由于 ELF 目标文件不需要被装载，所以它没有程序头表，而 ELF 的可执行文件和共享库文件都有。
