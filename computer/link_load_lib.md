...
- [第 1 部分简介](#第-1-部分简介)
  - [第 1 章 温故而知新](#第-1-章-温故而知新)
- [第 2 部分 静态链接](#第-2-部分-静态链接)
  - [第 2 章 编译和链接](#第-2-章-编译和链接)
    - [2.1 被隐藏的过程](#2.1-被隐藏的过程)
    - [2.2 编译器做了什么](#2.2-编译器做了什么)
    - [链接器](#链接器)
    - [静态链接](#静态链接)
  - [第 3 章 目标文件里有什么](#第-3-章-目标文件里有什么)
    - [3.1 目标文件的格式](#3.1-目标文件的格式)
    - [3.2 目标文件是什么样的？](#3.2-目标文件是什么样的？)
    - [3.4 ELF 文件结构描述](#3.4-elf-文件结构描述)
    - [3.5 链接的接口——符号](#3.5-链接的接口——符号)
    - [4.6 调试信息](#4.6-调试信息)
  - [第 4 章 静态链接](#第-4-章-静态链接)
    - [4.1 空间与地址分配](#4.1-空间与地址分配)
    - [4.2 符号解析与重定位](#4.2-符号解析与重定位)
    - [4.3 COMMON 块](#4.3-common-块)
    - [4.4 C++ 相关问题](#4.4-c++-相关问题)
    - [4.5 静态库链接](#4.5-静态库链接)
    - [4.6 链接过程控制](#4.6-链接过程控制)
    - [4.7 BFD 库](#4.7-bfd-库)
  - [第 5 章 Windows PE/COFF](#第-5-章-windows-pe/coff)
    - [5.1 Windows 的二进制文件格式 PE/COFF](#5.1-windows-的二进制文件格式-pe/coff)
    - [5.2 PE 的前身——COFF](#5.2-pe-的前身——coff)
    - [5.6 Windows 下的 ELF——PE](#5.6-windows-下的-elf——pe)
- [第 3 部分 装载与动态链接](#第-3-部分-装载与动态链接)
  - [第 6 章 可执行文件的装载与进程](#第-6-章-可执行文件的装载与进程)
    - [6.1 进程虚拟地址空间](#6.1-进程虚拟地址空间)
    - [6.2 装载的方式](#6.2-装载的方式)
    - [6.3 从操作系统角度看可执行文件的装载](#6.3-从操作系统角度看可执行文件的装载)
    - [进程虚存空间分布](#进程虚存空间分布)
    - [6.5 Linux 内和装载 ELF 过程简介](#6.5-linux-内和装载-elf-过程简介)
    - [6.6 Windows PE 的装载](#6.6-windows-pe-的装载)
  - [第 7 章 动态链接](#第-7-章-动态链接)
    - [7.3 地址无关代码](#7.3-地址无关代码)
    - [7.4 延迟绑定（PLT）](#7.4-延迟绑定（plt）)
    - [7.5 动态链接相关结构](#7.5-动态链接相关结构)
    - [7.6 动态链接的步骤和实现](#7.6-动态链接的步骤和实现)
    - [7.7 显式运行时链接](#7.7-显式运行时链接)
  - [第 8 章 Linux 共享库的组织](#第-8-章-linux-共享库的组织)
    - [8.1 共享库版本](#8.1-共享库版本)
    - [8.3 共享库系统路径](#8.3-共享库系统路径)
    - [8.4 共享库查找过程](#8.4-共享库查找过程)
    - [8.5 环境变量](#8.5-环境变量)
    - [共享库的创建和安装](#共享库的创建和安装)
  - [第 9 章 Windows 下的动态链接](#第-9-章-windows-下的动态链接)
- [第 4 部分 库与运行库](#第-4-部分-库与运行库)
  - [第 10 章 内存](#第-10-章-内存)
    - [10.1 程序的内存布局](#10.1-程序的内存布局)
    - [10.2 栈与调用惯例](#10.2-栈与调用惯例)
    - [10.3 堆与内存管理](#10.3-堆与内存管理)
  - [第 11 章 运行库](#第-11-章-运行库)
    - [11.1 入口函数和程序初始化](#11.1-入口函数和程序初始化)
    - [11.2 C/C++ 运行库](#11.2-c/c++-运行库)
    - [11.3 运行库与多线程](#11.3-运行库与多线程)
    - [11.4 全局构造与析构](#11.4-全局构造与析构)
    - [11.5 fread 实现](#11.5-fread-实现)
  - [第 12 章 系统调用与 API](#第-12-章-系统调用与-api)
    - [12.2 系统调用原理](#12.2-系统调用原理)
  - [第 13 章 运行库实现](#第-13-章-运行库实现)
